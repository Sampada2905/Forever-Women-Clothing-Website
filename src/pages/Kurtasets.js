...
export default function Kurtasets() {
 ...
 function fetchproducts() {
     if (productsubcategory === "allsubcategory") {
       axios.get("http://localhost:6060/getproducts/kurtasets").then((response) => {
         setProducts(response.data)
       })
     }
     else{
       axios.get("http://localhost:6060/getproducts/kurtasets/" + productsubcategory).then((response) => {
         setProducts(response.data)
       })
     }
   }
  ...
  return (
    <div>
      <h1 style={{color:'crimson'}}>Kurta Sets</h1>
      <div className='row'>
