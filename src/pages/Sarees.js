...
export default function Sarees() {
  ...
  function fetchproducts() {
    if (productsubcategory === "allsubcategory") {
      axios.get("http://localhost:6060/getproducts/sarees").then((response) => {
        setProducts(response.data)
      })
    }
    else {
      axios.get("http://localhost:6060/getproducts/sarees/" + productsubcategory).then((response) => {
        setProducts(response.data)
      })
    }
  }
  ...
  return (
    <div>
      <h1 style={{ color: 'crimson' }}>Sarees</h1>
      <div className='row'>
